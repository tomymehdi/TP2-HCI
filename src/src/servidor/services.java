...
public class services {
		...
		new catalogRequest("GetCategoryList", "language_id=" + langid)
				.make(new RequestResponse() {
					...
					public void handle(Document response) {
						...
						for (int i = 0; i < catnodes.getLength(); i++) {
							categories.add(new Category(catnodes.item(i)
									.toString()));
						}
						 ...
						 * ObjectAction<List<Category>>(categories) {
						 * 
						 * public void run() {
	...
	public void loadSubcategories(int category_id) {
		...
		new catalogRequest("GetSubcategoryList", "language_id=" + langid
			...
			public void handle(Document response) {
				...
				for (int i = 0; i < catnodes.getLength(); i++)
					subcategories.add(new SubCategory((Element) catnodes
							.item(i)));

				/*
				 * SwingUtilities.invokeLater(new
				 * ObjectAction<List<Subcategory>>(subcategories) {
				 * 
				 * public void run() {
		...
		if (category_id != 0) {
			if (subcategory_id == 0) {
				...
				new catalogRequest("GetProductListBySubcategory",
						...
						.make(rcb);
				// mainFrame.stats.incrementStat("SubCategory-" +
				// subcategory_id);
			}
		}
	}
	...
	public void showProductDetail(int product_id) {
		// App.sidebar.showLoading();
		new catalogRequest("GetProduct", "product_id=" + product_id)
				.make(new RequestResponse() {
					public void handle(Document response) {

						Element element = (Element) response
						...
						/*
						 * for (String key : p.info.keySet()) { if
						 * (!"image_url".equals(key)) { message += "<b>" +
						 * context.getResourceMap().getString(key) + "</b> " +
						 * p.info.get(key) + "<br />"; } }
						 */
						// App.sidebar.showLogo();
						// JOptionPane.showMessageDialog(mainFrame, message,
						// p.name, JOptionPane.INFORMATION_MESSAGE, new
						// ImageIcon(p.image));
	...
	public void signIn(String userName, String pass) {
		...
		new securityRequest("SignIn", "username=" + userName + "&password="
			...
			public void handle(Document response) {
				String token = response.getElementsByTagName("token").item(0)
						.getNodeValue();
				String username = response.getElementById("user").getAttribute(
						"username");
				app.setToken(token);
	...
	public void loadAddress(String uname, String token, int addId) {
		...
		new orderRequest("GetAddressList", "username=" + uname
				+ "&authentication_token=" + token).make(new RequestResponse() {
