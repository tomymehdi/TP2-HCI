...
public class Request {
	...
	/*
	 * public void makeSync(RequestResponse rr) { make(rr, null, false); }
	 * 
	 * public void makeSync(RequestResponse rr, RequestError ec) { make(rr, ec,
	 * false); }
	 */

	/*
	 * public void make(final RequestResponse rc, final RequestError ec, boolean
	 * async) {
	 * 
	 * if (async) { thread = new Thread ( new Runnable() { public void run() {
	 * domake(rc, ec); } } );
	 * 
	 * thread.start();
	 * 
	...
	private void domake(RequestResponse rr, RequestError ec) {
		...
		try {
			 ...
			 * ByteArrayInputStream(this.response.getBytes("UTF-8"))) );
			 */
