...
import com.ar.art.objects.Address;
import com.ar.art.objects.App;
import com.ar.art.objects.BookItem;
import com.ar.art.objects.Category;
import com.ar.art.objects.Item;
import com.ar.art.objects.MovieItem;
import com.ar.art.objects.Order;
import com.ar.art.objects.SubCategory;
import com.ar.art.objects.User;
...
public class Services {
	...
	public void loadCategories() {
		...
		new CatalogRequest("GetCategoryList", "language_id=" + langid)
				.make(new RequestResponse() {
	...
	private void loadSubcategories(final int category_id) {
		...
		new CatalogRequest("GetSubcategoryList", "language_id=" + langid
				+ "&category_id=" + category_id).make(new RequestResponse() {
			public void handle(String response) {
	...
	public void loadItems(int category_id, int subcategory_id) {
		...
		if (category_id != 0) {
			if (subcategory_id == 0) {
				new CatalogRequest("GetProductListByCategory", "language_id="
						+ langid + "&category_id=" + category_id).make(rcb);
			} else {
				new CatalogRequest("GetProductListBySubcategory",
						"language_id=" + langid + "&category_id=" + category_id
	...
	private void loadInfo(final int id, String response) {
		...
		new CatalogRequest("GetProduct", "product_id=" + id)
				.make(new RequestResponse() {
		...
		try {
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				...
				Date release_date = Date
						.valueOf(getCharacterDataFromElement(line));
				...
				((MovieItem) item).addInfo(actors, format, lang, subtitles,
						region, aspect_ratio, number_discs, release_date,
						run_time, ASIN);
	...
	private void parseBookInfo(String response, Item item) {
		...
		try {
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				...
				((BookItem) item).addInfo(authors, publisher, pubdate, ISBN_10,
						ISBN_13, lang);
	...
	private List<Item> parseItems(String response) {
		...
		try {
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				...
				NodeList sales_rank = element
						.getElementsByTagName("sales_rank");
				...
				if (categ == 1) {
					ans.add(new MovieItem(id, sales, name, categ, scateg,
							price, imgUrl));
				} else {
					ans.add(new BookItem(id, sales, name, categ, scateg, price,
							imgUrl));
	...
	public void loadSearchResults(String query) {
		new CatalogRequest("GetProductListByName", "criteria=" + query)
				.make(new RequestResponse() {
					public void handle(String response) {
	...
	public void signIn(String userName, String pass) {
		...
		new SecurityRequest("SignIn", "username=" + userName + "&password="
				+ pass).make(new RequestResponse() {
	...
	public void signOut(String uname, String token) {
		...
		new SecurityRequest("SignOut", "username=" + uname
				+ "&authentication_token=" + token).make(new RequestResponse() {
	...
	public void loadOrders(String uname, String token) {
		...
		new OrderRequest("GetOrderList", "username=" + uname
				+ "&authentication_token=" + token).make(new RequestResponse() {
				...
				try {
					...
					for (int i = 0; i < nodes.getLength(); i++) {
						...
						if (status == 2) {
							...
							Order order = new Order(id, created_date,
									confirmed_date);
	...
	private void getAddress(int id, final Order order) {
		...
		new OrderRequest("GetAddress", "username="
				+ app.getUser().getUserName() + "&authentication_token="
				+ app.getToken() + "&address_id=" + id)
				.make(new RequestResponse() {
